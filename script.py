...
def load_config() -> dict:
    ...
    try:
        ...
        print(f"[CONFIG] {CONFIG_PATH} not found!", flush=True)
        return {}
...
def analyze_pdf(file_bytes: bytes) -> list:
    ...
    try:
        ...
        checks = [
            # 1. فحص التشغيل التلقائي: لو الفيروس بيشغل نفسه أول ما تفتح الملف (مهم جداً وشغال)
            ("/OpenAction" in text and (" /JavaScript" in text or "/JS" in text),
             "PDF auto-runs JavaScript on open"),
            
            # 2. فحص وجود ملفات مدمجة مخفية
            ("/EmbeddedFile" in text,
             "PDF contains embedded files"),
            
            # 3. فحص أوامر تشغيل برامج النظام الخارجية
            ("/Launch" in text,
             "PDF contains Launch action (can execute programs)"),
            
            # 4. فحص الروابط الخارجية الكثيرة (أكبر من 10)
            (text.count("/URI") > 10,
             f"PDF has {text.count('/URI')} external links (suspicious)"),
            
            # 5. فحص ثغرات الـ XFA المتقدمة
            ("/XFA" in text,
             "PDF XFA form (advanced exploit vector)"),
            
            # 6. فحص فلاتر الضغط المفرطة (تعديلك الـ 700 شغال ومظبوط)
            ("/FlateDecode" in text and text.count("/Filter") > 700,
